...
Use an `async` function that `await`s the result of a `fetch` call to that route.
...
When you click on this `button`, you should fire your `fetch` request to draw five cards.
...
In your button event listener, after your `fetch` request, loop through your five cards.
